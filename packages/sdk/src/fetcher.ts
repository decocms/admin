import type { GlobalTools, MCPClientFetchStub, ToolBinder, WorkspaceTools } from './mcp/index.ts';
  ...
  },
  {
    get(_, name) {
      if (name === 'forWorkspace') {
        return (workspace: string) => createMCPFetchStub<WorkspaceTools>({ workspace });
      }
