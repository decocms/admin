...
// Internal form data type with simplified tools format
interface InternalRoleFormData {
  name: string;
  description?: string;
  tools: Record<string, string[]>; // Simplified internal format
  agents: string[];
  members: string[];
}
...
interface AddRoleDialogProps {
  ...
  role?: TeamRole;
  teamId?: number;
  onSave: (roleData: InternalRoleFormData, isEditing: boolean) => void;
  initialTab?: string;
}
...
interface RoleGeneralPanelProps {
  formData: InternalRoleFormData;
  setFormData: (
    data:
      | InternalRoleFormData
      | ((prev: InternalRoleFormData) => InternalRoleFormData),
  ) => void;
}
...
const DEFAULT_ROLES: DefaultRole[] = [
  ...
  },
  {
    ...
    name: "Manager",
    description:
      "Manages resources, agents, and integrations, but no access to billing.",
  },
  {
    id: 3,
    name: "Builder",
    description:
      "Creates and maintains agents, integrations, and automations, without user or billing management.",
  },
  {
    id: 4,
    name: "Member",
    description:
      "Collaborates in threads and uses agents and integrations, with no administrative permissions.",
  },
];
...
function RoleGeneralPanel({ formData, setFormData }: RoleGeneralPanelProps) {
  const handleNameChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    setFormData((prev: InternalRoleFormData) => ({
      ...prev,
      name: e.target.value,
    }));
  }, [setFormData]);
...
function RoleToolsPanel({
  ...
  const handleSelectAll = useCallback(async () => {
    const hasAnySelected = Object.values(formData.tools)
      .some((tools) => Array.isArray(tools) && tools.length > 0);

    if (hasAnySelected) {
      setFormData((prev: InternalRoleFormData) => ({ ...prev, tools: {} }));
    } else {
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {selectingAllTools
                  ? "Loading..."
                  : Object.values(formData.tools).some((tools) =>
...
function RoleDialogIntegrationListItem({
  ...
  const handleAll = useCallback((checked: boolean) => {
    setIntegrationTools(
      integration.id,
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            variant="outline"
            size="sm"
            disabled={isLoading || toolsInfo.total === 0}
                ...
                >
                  <div className="flex flex-col min-w-0 flex-1">
                    <span className="text-sm font-medium truncate text-foreground">
                      {tool.name.replace(/_/g, " ").toLowerCase().replace(
                        /\b\w/g,
...
function AddRoleDialog({
  ...
  // Convert external role to internal format for editing
  const convertToInternal = (role?: TeamRole): InternalRoleFormData => {
    if (!role) {
      return {
        name: "",
        description: "",
    ...
    // Convert tools from API format to internal format
    const internalTools: Record<string, string[]> = {};
    if (role.tools) {
      Object.entries(role.tools).forEach(([integrationId, tools]) => {
        internalTools[integrationId] = tools.map((tool) =>
  ...
  const [formData, setFormData] = useState<InternalRoleFormData>(() =>
    convertToInternal(role)
  );
    ...
    () =>
      availableIntegrations.filter((integration) =>
        integration.name.toLowerCase().includes(
          deferredToolsSearch.toLowerCase(),
        )
      ),
  ...
  const filteredAgents = useMemo(
    () =>
      agents.filter((agent) =>
        agent.name.toLowerCase().includes(deferredAgentsSearch.toLowerCase()) ||
        (agent.description || "").toLowerCase().includes(
          deferredAgentsSearch.toLowerCase(),
        )
      ),
  ...
  const handleAgentToggle = useCallback((agentId: string, checked: boolean) => {
    if (checked) {
      ...
      Object.keys(agentTools).forEach((integrationId) => {
        if (
          !roleTools[integrationId] || roleTools[integrationId].length === 0
        ) {
  ...
  useEffect(() => {
    if (role) {
      setFormData({
        name: role.name || "",
        description: role.description || "",
        tools: Object.fromEntries(Object.entries(role.tools || {}).map(([integrationId, tools]) => [
          integrationId,
          tools.map((tool) =>
            typeof tool === "string" ? tool : tool.toolName
          ),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-fit min-w-[900px] max-w-[95vw] max-h-[80vh] overflow-hidden">
        ...
        <div className="flex h-[60vh]">
          ...
          <div
            ref={contentAreaRef}
            className="flex-1 pl-6 overflow-y-auto min-w-0"
          >
            {selectedTab === "general" && (
...
export function RolesTableView({
  ...
  const [editingRole, setEditingRole] = useState<TeamRole | undefined>(undefined);
  const [roleDialogInitialTab, setRoleDialogInitialTab] = useState("general");

  // Utility Functions
  function mapRoleDataToFormData(roleData: InternalRoleFormData): RoleFormData {
    return {
      ...roleData,
      tools: Object.fromEntries(
        Object.entries(roleData.tools).map(([integrationId, tools]) => [
          integrationId,
          tools.map((tool) =>
            typeof tool === "string"
              ? { toolName: tool, effect: "allow" as const }
              : tool
          ),
        ]),
      ),
  ...
  const handleCreateRole = useCallback(
    async (roleData: InternalRoleFormData) => {
      if (!teamId) return;
      try {
        await createRoleMutation.mutateAsync({
          teamId,
          roleData: mapRoleDataToFormData(roleData),
        });
  ...
  const handleUpdateRole = useCallback(
    async (roleId: number, roleData: InternalRoleFormData) => {
      if (!teamId) return;
      try {
  ...
  const handleRolesSort = useCallback((key: string) => {
    if (rolesSortKey === key) {
  ...
  const getRoleSortValue = useCallback(
    (role: TeamRole, key: string): string => {
      switch (key) {
        case "role":
  ...
  const handleEditRole = useCallback(
    (role: TeamRole | undefined, initialTab = "general") => {
      setEditingRole(role);
  ...
  const rolesColumns = useMemo(() => [
    ...
    },
    {
      ...
      render: (role: TeamRole) => {
        ...
        const roleAgentsList = agents.filter((agent) =>
          roleAgents.includes(agent.id)
        );
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem
              ...
              onClick={async () => {
                if (!DEFAULT_ROLES.some((r) => r.id === role.id) && teamId) {
                  try {
  ...
  ] satisfies TableColumn<TeamRole>[], [
    agents,
    integrations,
    handleEditRole,
    members,
  ...
  const filteredRoles: TeamRole[] = useMemo(
    () =>
      (roles as unknown as TeamRole[]).filter((role) =>
    ...
    <>
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          />
          <Button
            onClick={() => handleEditRole(undefined)}
          >
            <Icon name="add" />
            Add role
          </Button>
      ...
      <AddRoleDialog
        ...
        onSave={async (roleData, isEditing) => {
