...
import { getWorkflowStatus, listWorkflowNames, listWorkflowRuns } from '../crud/workflows.ts';
...
export const useWorkflowStatus = (workflowName: string, instanceId: string) => {
  ...
  return useSuspenseQuery({
    ...
    queryFn: ({ signal }) => getWorkflowStatus(workspace, { workflowName, instanceId }, signal),
    retry: (failureCount, error) => error instanceof InternalServerError && failureCount < 2,
