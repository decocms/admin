...
import { selectAnyOfSchema, getDetectedType, doesSchemaTypeMatchValue } from "./schema.ts";
...
export function generateDefaultValue(
  ...
  if (schema.type && typeof schema.type === 'string' && 
    ...
    if (formData !== undefined && formData !== null) {
      ...
      const schemaTypeMatches = doesSchemaTypeMatchValue(schema.type, formDataType);
  ...
  switch (type) {
    case "string":
      return schema.default !== undefined ? schema.default : 
             schema.enum && schema.enum.length > 0 ? schema.enum[0] : "";
    case "number":
    case "integer":
      ...
      if (schema.properties) {
        ...
        for (const [name, propSchema] of Object.entries(schema.properties)) {
          ...
          if (childFormData !== undefined) {
            ...
            const typeMatches = doesSchemaTypeMatchValue((propSchema as JSONSchema7).type, childType);
            ...
            if (typeMatches) {
              ...
              result[name] = generateDefaultValue(propSchema as JSONSchema7, undefined, propPath);
            }
