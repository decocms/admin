...
function getNextFridayAt2PM(): { startDate: Date; endDate: Date } {
  ...
  const startDate = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 14, 0, 0);
  const endDate = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 15, 0, 0);
  
  return { startDate, endDate };
}

const { startDate: COMMUNITY_CALL_START_DATE, endDate: COMMUNITY_CALL_END_DATE } = getNextFridayAt2PM();
