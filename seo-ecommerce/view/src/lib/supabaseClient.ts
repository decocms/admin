...
async function dynamicCreateClient(): Promise<(url: string, anon: string, opts?: any)=>SupabaseClient> {
  ...
  const cdn: any = await import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm');
...
export async function loadSupabase() {
  ...
  // Preload session
  try { client.auth.getSession().then((r: any)=>{ cachedSession = r?.data?.session || null; }); } catch {}
  client.auth.onAuthStateChange?.((_e: any, session: Session)=>{ cachedSession = session; });
  return client;
}
