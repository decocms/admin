...
import { InstallModal } from "./install-modal.tsx";
...
const mockItemData = {
  "deco-cx": {
    ...
    fullDescription: "deco.cx is a powerful platform that provides everything you need to build modern web applications. It includes a comprehensive page builder, section management system, and data loaders that make it easy to create dynamic, performant websites. Perfect for e-commerce, content sites, and web applications.",
    iconName: "check",
    ...
    features: [
      { name: "Page Builder", description: "Visual page builder with drag-and-drop", icon: "layers" },
      { name: "Sections", description: "Reusable section components", icon: "layout_panel_top" },
      { name: "Data Loaders", description: "Dynamic data loading system", icon: "box" },
      { name: "E-commerce Ready", description: "Built-in e-commerce features", icon: "shopping_cart" }
    ],
    pricing: {
      type: "free",
      description: "Free to use with optional premium features.",
      plans: [
        { name: "Free Plan", price: "Free", unit: "forever", features: ["All core features", "Unlimited pages", "Basic support"] },
        { name: "Pro Plan", price: "$29", unit: "per month", features: ["All Free features", "Advanced analytics", "Priority support", "Custom domains"] },
        { name: "Enterprise", price: "Custom", unit: "pricing", features: ["All Pro features", "SLA guarantee", "Dedicated support", "Custom integrations"] }
      ]
    },
    changelog: [
      {
        version: "v3.0.0",
        date: "2 hours ago",
        changes: ["New visual page builder", "Improved performance", "Added e-commerce templates", "Enhanced loader system"]
      },
      {
        version: "v2.5.0",
        date: "1 week ago",
        changes: ["Section library expansion", "Better TypeScript support", "New data connectors"]
      }
    ]
  },
  "sheets-creator": {
  ...
  "crawler": {
    ...
    features: [
      { name: "Smart Parsing", description: "AI-powered content extraction", icon: "brain" },
      { name: "Rate Limiting", description: "Respectful crawling with delays", icon: "timer" },
      { name: "Format Preservation", description: "Maintains original formatting", icon: "description" },
  ...
  "notion-tool": {
    ...
    features: [
      { name: "Database Creation", description: "Create databases with custom properties", icon: "storage" },
  ...
  "calendar-agent": {
    ...
    features: [
      { name: "Smart Scheduling", description: "AI-powered meeting scheduling", icon: "brain" },
  ...
  "famous3": {
    ...
    features: [
      ...
      { name: "Multi-Calendar", description: "Sync across multiple calendar platforms", icon: "schedule" },
  ...
  "famous4": {
    ...
    features: [
      { name: "Data Processing", description: "Process large datasets efficiently", icon: "brain" },
      { name: "AI Insights", description: "Generate intelligent data insights", icon: "auto_awesome" },
      { name: "Visualizations", description: "Create charts and graphs", icon: "trending_up" },
  ...
  "famous5": {
    ...
    features: [
      { name: "Multi-Format", description: "Generate content in various formats", icon: "description" },
  ...
  "famous6": {
    ...
    features: [
      { name: "Workflow Builder", description: "Visual workflow creation", icon: "settings" },
      { name: "Scheduling", description: "Time-based task execution", icon: "timer" },
  ...
  "famous7": {
    ...
    features: [
      { name: "OCR Technology", description: "Advanced optical character recognition", icon: "visibility" },
      { name: "Multi-Format", description: "Support for PDF, Word, images", icon: "description" },
      { name: "Data Extraction", description: "Extract structured data", icon: "storage" },
  ...
  "famous8": {
    ...
    features: [
      ...
      { name: "Scheduling", description: "Advanced post scheduling", icon: "timer" },
      { name: "Analytics", description: "Detailed engagement analytics", icon: "trending_up" },
  ...
  "famous9": {
    ...
    features: [
      { name: "AI Analysis", description: "Intelligent image analysis and tagging", icon: "brain" },
...
export function ItemDetailPage() {
  ...
  const { installApp, uninstallApp, isAppInstalled } = useInstalledApps();
  const [showInstallModal, setShowInstallModal] = useState(false);
  const [isHoveringButton, setIsHoveringButton] = useState(false);
  
  const item = mockItemData[itemId as keyof typeof mockItemData];
  const isInstalled = item ? isAppInstalled(item.id) : false;
  
  // Listen for app installation events
  useEffect(() => {
    const handleAppInstalled = (event: CustomEvent) => {
      if (event.detail.id === item?.id) {
        // Force re-render when this app is installed
        window.location.reload();
      }
    };
    
    window.addEventListener("app-installed" as any, handleAppInstalled);
    return () => {
      window.removeEventListener("app-installed" as any, handleAppInstalled);
    };
  }, [item?.id]);
  ...
  const handleUninstall = () => {
    uninstallApp(item.id);
    toast.success(`${item.name} has been uninstalled.`);
    
    // If it's deco.cx, reload the page to update sidebar
    if (item.id === "deco-cx") {
      setTimeout(() => {
        window.location.reload();
      }, 500);
  ...
  const handleInstallConfirm = () => {
    ...
    toast.success(`${item.name} has been installed successfully!`);
    
    // If it's deco.cx, reload the page to show it in the sidebar
    if (item.id === "deco-cx") {
      setTimeout(() => {
        window.location.reload();
      }, 1000);
    ...
    <div className="flex flex-col gap-2 p-1 w-full min-h-screen">
      ...
      <div className="flex flex-row gap-2 items-start justify-center max-w-[1200px] mx-auto w-full">
        <div className="flex-1 max-w-[900px] p-4 space-y-12 pb-16">
              ...
              <div className="flex flex-row gap-20 items-center justify-start">
                <div className="flex flex-row gap-4 items-center flex-1">
                  ...
                  <div className={cn("w-32 h-32 rounded-3xl flex items-center justify-center overflow-hidden", item.iconBg)}>
                    {item.iconUrl ? (
                      <img src={item.iconUrl} alt={item.name} className="w-20 h-20 object-contain" />
                    ) : (
                  ...
                  <div className="flex flex-col gap-2 flex-1">
                    <h1 className="text-xl font-medium text-foreground">{item.name}</h1>
                ...
                <div className="flex flex-row gap-1 items-center">
                  <Button 
                    variant={isInstalled ? "outline" : "special"}
                    className={cn(
                      "px-8 py-2 rounded-xl text-sm transition-all duration-200",
                      isInstalled && isHoveringButton && "bg-destructive text-destructive-foreground border-destructive hover:bg-destructive/90"
                    )}
                    onClick={isInstalled && isHoveringButton ? handleUninstall : handleInstall}
              ...
              <div className="flex flex-row items-center justify-between w-full">
                      ...
                      {item.authorBadges.map((badge, index) => (
                        <div key={badge} className="flex h-[18px] w-[18px] items-center justify-center -mr-2 last:mr-0 relative">
                          <div className="flex-none rotate-45">
                            <div 
                              className="w-[13px] h-[13px] rounded-sm border-2 border-border"
                              style={{ 
                ...
                <div className="flex flex-col gap-2.5 items-center">
                  ...
                  <div className="text-center">
                    <p className="text-sm text-foreground font-normal">{item.installs.toLocaleString()}</p>
                    <p className="text-sm text-foreground font-normal">installs</p>
                </div>
              </div>

                {/* Updated */}
                <div className="flex flex-col gap-2.5 items-center">
                  <Icon name="update" size={18} className="text-muted-foreground" />
                  <div className="text-center">
                    <p className="text-sm text-foreground font-normal">Updated</p>
                    <p className="text-sm text-foreground font-normal">{item.updatedAt}</p>
                  </div>
              </div>
                ...
                <div className="flex flex-col gap-2.5 items-center">
                  ...
                  <div className="text-center">
                    <p className="text-sm text-foreground font-normal">{item.monthlyActiveUsers} Monthly</p>
                    <p className="text-sm text-foreground font-normal">active users</p>
                </div>
                </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <h2 className="text-base font-medium text-foreground">What's included</h2>
            
            <div className="flex flex-wrap gap-2">
              {item.features.map((feature, index) => (
                <div
                  key={index}
                  className="flex flex-row gap-2.5 items-center p-4 border border-border rounded-2xl w-[430px]"
                >
                  ...
                  <span className="text-sm text-muted-foreground flex-1">{feature.description}</span>
                </div>
              ))}
        </div>
      </div>
              ...
              <TabsContent value="pricing" className="space-y-8 mt-8">
                ...
                <div className={`grid gap-6 ${item.pricing.plans.length === 1 ? 'grid-cols-1 max-w-md' : item.pricing.plans.length === 2 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-3'}`}>
                  {item.pricing.plans.map((plan, index) => (
                    <div
                      key={index}
                      className={`flex flex-col p-6 border border-border rounded-2xl hover:border-muted transition-colors ${
                    ...
                    >
                      ...
                      <div className="text-center mb-6">
                        ...
                        <div className="flex items-baseline justify-center gap-1 mb-2">
                          ...
                          {plan.unit && <span className="text-sm text-muted-foreground ml-1">{plan.unit}</span>}
                  </div>
                  ...
                  <div className="p-6 border border-border rounded-2xl">
                    <div className="space-y-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-medium text-foreground">Top Builders Using This App</h3>
                    </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-xl">
                          ...
                          <div className="flex-1">
                            <p className="text-sm font-medium text-foreground">Leandro Borges</p>
                            <p className="text-xs text-muted-foreground">15 bounties completed</p>
                          </div>
                        ...
                        {/* Builder 2 */}
                        <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-xl">
                          <div className="flex items-center justify-center w-6 h-6 bg-muted text-white text-xs font-bold rounded-full">
                            2
                          </div>
                          <img 
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">12 bounties completed</p>
                          </div>
                          <div className="flex items-center gap-1">
                            <Icon name="star" size={12} className="text-yellow-500" />
                        ...
                        <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-xl">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">9 bounties completed</p>
                          </div>
                          <div className="flex items-center gap-1">
                            <Icon name="star" size={12} className="text-yellow-500" />
                        ...
                        {/* Builder 4 */}
                        <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-xl">
                          <div className="flex items-center justify-center w-6 h-6 bg-muted text-foreground text-xs font-bold rounded-full">
                            4
                          </div>
                          <img 
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">7 bounties completed</p>
                          </div>
                          <div className="flex items-center gap-1">
                            <Icon name="star" size={12} className="text-yellow-500" />
      ...
      <InstallModal
        ...
        appName={item.name}
        appDescription={item.fullDescription}
