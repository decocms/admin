...
function log(...m) {
  if (verbose) console.log('[wrangler-deploy-auto]', ...m);
}

function pickCommand() {
  if (fs.existsSync(localBin)) {
    return { cmd: localBin, args: ['deploy', ...passThroughArgs] };
  }
  if (!process.env.NO_NPX_FALLBACK) {
    return { cmd: npxCmd, args: ['wrangler', 'deploy', ...passThroughArgs] };
  }
  return {
    cmd: isWin ? 'wrangler.cmd' : 'wrangler',
    args: ['deploy', ...passThroughArgs],
  };
}
