...
import { Avatar, AvatarFallback, AvatarImage } from '@deco/ui/components/avatar.tsx';
...
export default function HistoryTab() {
  ...
  const { form, prompt, setSelectedPrompt, promptVersion, setPromptVersion } = useFormContext();
...
export function HistoryCard({
  ...
  const { prompt, setSelectedPrompt, promptVersion, setPromptVersion } = useFormContext();
  ...
  const member = !isCurrentUser && teamId !== null ? teamMembersMap.get(userId) : undefined;

  const avatarUrl = isCurrentUser
    ? user.metadata.avatar_url
    : member?.profiles?.metadata?.avatar_url;
  const userName = isCurrentUser ? user.metadata.full_name : member?.profiles?.metadata?.full_name;
