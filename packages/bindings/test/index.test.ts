...
describe("@decocms/bindings", () => {
  ...
  describe("RegExp tool name matching", () => {
    const REGEX_BINDING = [
      {
        name: /^RESOURCE_\w+_SEARCH$/ as RegExp,
        inputSchema: z.object({ term: z.string() }),
        outputSchema: z.object({ items: z.array(z.any()) }),
      },
      {
        name: /^RESOURCE_\w+_READ$/ as RegExp,
        inputSchema: z.object({ uri: z.string() }),
        outputSchema: z.object({ data: z.any() }),
      },
    ] as const satisfies Binder;

    it("should match tools using RegExp patterns", async () => {
      const checker = createBindingChecker(REGEX_BINDING);

      const tools = [
        {
          name: "RESOURCE_WORKFLOW_SEARCH",
          inputSchema: z.object({ term: z.string() }),
          outputSchema: z.object({ items: z.array(z.any()) }),
        },
        {
          name: "RESOURCE_WORKFLOW_READ",
          inputSchema: z.object({ uri: z.string() }),
          outputSchema: z.object({ data: z.any() }),
        },
      ];
    ...
    it("should match different resource types", async () => {
      const checker = createBindingChecker(REGEX_BINDING);

      const tools = [
        {
          name: "RESOURCE_USER_SEARCH",
          inputSchema: z.object({ term: z.string() }),
          outputSchema: z.object({ items: z.array(z.any()) }),
        },
        {
          name: "RESOURCE_USER_READ",
          inputSchema: z.object({ uri: z.string() }),
          outputSchema: z.object({ data: z.any() }),
        },
      ];
