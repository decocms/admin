...
console.log('[verify-scripts] Running script and documentation consistency checks...');
...
for (const scriptName of criticalScripts) {
  if (!scripts[scriptName]) {
    warn(`Script "${scriptName}" is considered critical but not found in package.json.`);
    continue;
  }
  const regex = new RegExp(`\`${scriptName}\``, 'g');
  if (!regex.test(readme)) {
    warn(`Critical script "${scriptName}" is not documented in README.md inside backticks.`);
  }
}
