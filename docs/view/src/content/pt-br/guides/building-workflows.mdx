...
Workflows orquestram múltiplas tools em sequência para automatizar processos complexos. Diferente de agentes de IA que decidem quais tools chamar, workflows seguem passos determinísticos. Workflows no deco.chat usam o framework Mastra por baixo dos panos, dando a você um conjunto de padrões componíveis para fluxo de controle.

## Básico de Workflows

Um workflow encadeia tools e outros workflows em uma sequência determinística. Defina um com `createWorkflow()`, especifique schemas, encadeie steps com `.then()` e finalize com `.commit()`.
...
1. Go to your workspace in deco.chat
