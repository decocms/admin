import { NotFoundError, useAgentData, useFile, WELL_KNOWN_AGENTS } from '@deco/sdk';
        ...
        <div className='justify-self-start flex items-center gap-3 text-muted-foreground w-full'>
          {chat.messages.length > 0 && (
            <div className='flex justify-between items-center gap-2 w-full'>
              ...
              <Button
                className={messages.length > 0 && !hasChanges ? 'inline-flex text-xs' : 'hidden'}
                variant='outline'
                size='sm'
                onClick={() =>
...
function FormProvider(props: Props & { agentId: string; threadId: string }) {
  ...
  useDocumentMetadata({
    ...
    description: agent ? (agent.description ?? agent.instructions ?? '') : undefined,
