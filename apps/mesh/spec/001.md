...
// storage/token-revocation.ts
import type { BunSQLiteDatabase } from 'drizzle-orm/bun-sqlite';
import { eq, and, lte } from 'drizzle-orm';
import * as schema from './schema';

export class TokenRevocationStorage {
  constructor(private drizzle: BunSQLiteDatabase<typeof schema>) {}
    ...
    const [result] = await this.drizzle
      ...
      .where(
        and(
          ...
          lte(new Date(), schema.revokedTokens.expiresAt)
        )
      )
      .limit(1);
  ...
  async cleanExpired(): Promise<number> {
    const result = await this.drizzle
      ...
      .where(lte(schema.revokedTokens.expiresAt, new Date()));
...
interface Statement {
  ...
  actions?: string[]; // Optional: ['execute', 'read', etc.]
  matchCondition?: {
    resource: 'is_connection';
    connectionId: string; // Scope to specific connection ID
  };
}
