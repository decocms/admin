import { SupabaseClient } from "@supabase/supabase-js";
import crypto from "node:crypto";
import { Buffer } from "node:buffer";

export interface LLMVault {
  storeApiKey(
    modelId: string,
    workspace: string,
    apiKey: string,
  ): Promise<void>;
  getApiKey(
    modelId: string,
    workspace: string,
  ): Promise<{ apiKey: string | null; model: string } | null>;
  updateApiKey(
    modelId: string,
    workspace: string,
    apiKey: string | null,
  ): Promise<void>;
  removeApiKey(modelId: string, workspace: string): Promise<void>;
}

export class SupabaseLLMVault implements LLMVault {
  private encryptionKey: Buffer;
  private ivLength = 16; // AES block size

  constructor(
    private db: SupabaseClient,
    encryptionKey: string,
  ) {
    // console.log("encryptionKey", encryptionKey);
    if (encryptionKey.length !== 32) {
      throw new Error("Encryption key must be 32 characters long for AES-256");
    }
    this.encryptionKey = Buffer.from(encryptionKey);
  }

  private encrypt(text: string): string {
    const iv = crypto.randomBytes(this.ivLength);
    const cipher = crypto.createCipheriv("aes-256-cbc", this.encryptionKey, iv);
    let encrypted = cipher.update(text, "utf8", "hex");
    encrypted += cipher.final("hex");
    return iv.toString("hex") + ":" + encrypted;
  }

  private decrypt(encryptedText: string): string {
    const [ivHex, encrypted] = encryptedText.split(":");
    const iv = Buffer.from(ivHex, "hex");
    const decipher = crypto.createDecipheriv(
      "aes-256-cbc",
      this.encryptionKey,
      iv,
    );
    let decrypted = decipher.update(encrypted, "hex", "utf8");
    decrypted += decipher.final("utf8");
    return decrypted;
  }

  async storeApiKey(
    modelId: string,
    workspace: string,
    apiKey: string,
  ): Promise<void> {
    const encryptedKey = this.encrypt(apiKey);

    const { error } = await this.db
      .from("models")
      .update({ api_key_hash: encryptedKey })
      .eq("id", modelId)
      .eq("workspace", workspace);

    if (error) throw error;
  }

  async getApiKey(
    modelId: string,
    workspace: string,
  ): Promise<{ apiKey: string | null; model: string } | null> {
    const { data, error } = await this.db
      .from("models")
      .select("api_key_hash, model")
      .eq("id", modelId)
      .eq("workspace", workspace)
      .single();

    if (error) throw error;

    if (!data?.api_key_hash) return null;

    return {
      apiKey: this.decrypt(data.api_key_hash),
      model: data.model,
    };
  }

  async updateApiKey(
    modelId: string,
    workspace: string,
    apiKey: string | null,
  ): Promise<void> {
    const encryptedKey = apiKey ? this.encrypt(apiKey) : null;

    const { error } = await this.db
      .from("models")
      .update({ api_key_hash: encryptedKey })
      .eq("id", modelId)
      .eq("workspace", workspace);

    if (error) throw error;
  }

  async removeApiKey(modelId: string, workspace: string): Promise<void> {
    const { error } = await this.db
      .from("models")
      .update({ api_key_hash: null })
      .eq("id", modelId)
      .eq("workspace", workspace);

    if (error) throw error;
  }
}

export function createHashToken(
  vault: LLMVault,
  modelId: string,
  workspace: string,
) {
  return async (token: string): Promise<string> => {
    await vault.storeApiKey(modelId, workspace, token);
    return "encrypted"; // Return a placeholder since we're storing in DB
  };
}
