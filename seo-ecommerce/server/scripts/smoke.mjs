...
async function run() {
  ...
  if (!ready) {
    console.error(`Worker not reachable at ${BASE} after ${MAX_WAIT_MS}ms. Did you start 'npm run dev' (or dev:all)?`);
    process.exit(1);
  }
  ...
  await step('list tools', async () => {
    ...
    if (!names.includes('LINK_ANALYZER')) throw new Error('LINK_ANALYZER missing');
  ...
  await step('LINK_ANALYZER', async () => {
    ...
    summary.tools.LINK_ANALYZER = { linkCount: Array.isArray(c.links) ? c.links.length : 0 };
  ...
  await step('PAGESPEED mobile', async () => {
    ...
    const r = await safeFetch(`${BASE}/mcp/tools`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ tool: 'PAGESPEED', input: { url: testUrl, strategy: 'mobile', category: ['performance','seo'] } }),
    });
  ...
  await step('PAGESPEED desktop', async () => {
    ...
    const r = await safeFetch(`${BASE}/mcp/tools`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ tool: 'PAGESPEED', input: { url: testUrl, strategy: 'desktop', category: ['performance','seo'] } }),
    });
