...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    mcp({
      ...
      resource: process.env.MCP_RESOURCE_URL || "https://mesh.example.com",
    }),
...
Better Auth automatically exposes these routes, but you should add explicit route handlers for better CORS compatibility:
...
Use Better Auth's `withMcpAuth` helper or `auth.api.getMcpSession` to handle MCP client authentication:

```typescript
// api/mcp/:connectionId/route.ts (Option 1: Using withMcpAuth)
import { auth } from "@/lib/auth";
...
const handler = withMcpAuth(auth, async (req, session) => {
  ...
  // - session.scopes: OAuth scopes granted
  // - session.accessToken: Access token details
  
  const connectionId = new URL(req.url).pathname.split('/').pop();
  const ctx = await createMeshContext(req, session);
...
```typescript
// api/mcp/:connectionId/route.ts (Option 2: Using getMcpSession)
import { auth } from "@/lib/auth";
...
export async function POST(req: Request) {
  ...
  if (!session) {
    // Important: Return 401 for unauthenticated requests
...
app.post('/mcp/:connectionId', requireMcpAuth(['mcp:tools']), async (c) => {
