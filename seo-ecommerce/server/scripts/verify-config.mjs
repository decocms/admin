...
if (!/id\s*=\s*"[a-f0-9]{32}"/i.test(kvSeo)) {
  fail("SEO_CACHE id missing or malformed");
}
if (!/preview_id\s*=\s*"[a-f0-9]{32}"/i.test(kvSeo)) {
  console.warn(
    "[verify-config] WARN: preview_id missing or malformed (ok, but recommended)",
  );
}
