  ...
  async list(projectId: string | null): Promise<MCPConnection[]> {
    ...
    const query = projectId
      ? and(
          ...
          // OR organization-scoped (projectId is null)
        )
...
export function createMeshContextFactory(
  ...
  return async (c: Context): Promise<MeshContext> => {
    ...
    if (projectSlug) {
      ...
      // Cluster-scoped (organization-level) request
      // Verify token audience is organization-scoped
      if (auth.token && auth.token.aud !== 'organization') {
        throw new UnauthorizedError(
          `Token audience mismatch: expected "organization" for cluster-level operations, got ${auth.token.aud}`
        );
      }
    }
