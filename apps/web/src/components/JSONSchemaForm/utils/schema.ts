...
export function getDetectedType(value: SchemaType): string {
  ...
  return valueType === 'object' && value === null ? 'null' :
         valueType === 'object' && Array.isArray(value) ? 'array' :
         valueType;
}
 ...
 */
export function doesSchemaTypeMatchValue(schemaType: string | string[] | undefined, detectedType: string): boolean {
 ...
 */
export function typeMatches(schemaType: JSONSchema7["type"], runtimeType: string): boolean {
  if (schemaType === undefined) {
...
export function findMatchingAnyOfSchema(
  ...
  const matchingSchema = schemas.find(subSchema => {
    ...
    if (checkProperties && detectedType === 'object' && 
    ...
    if (detectedType === 'array' && Array.isArray(value) && 
 ...
 */
export function findSchemaByChildFields<T extends FieldValues = Record<string, unknown>>(
  name: string,
  schemas: JSONSchema7[],
  ...
  const bestMatchSchema = schemas.find(subSchema => {
    for (const childPath of childFieldPaths) {
      if (doesChildFieldMatchSchema(childPath, namePrefix, subSchema as JSONSchema7)) {
        return true;
      }
    }
    return false;
 ...
 */
export function findSchemaByParentRelationship<T extends FieldValues = Record<string, unknown>>(
  name: string,
  schema: JSONSchema7,
  form: UseFormReturn<T>
): JSONSchema7 | undefined {
 ...
 */
export function selectAnyOfSchema<T extends FieldValues = Record<string, unknown>>(
  schema: JSONSchema7,
  ...
  if (form && fieldName) {
    ...
    const matchingSchema = findMatchingAnyOfSchema(schemas, currentValue as SchemaType, true);
    if (matchingSchema) {
      return matchingSchema;
    }
    
    // Try to match based on parent-child relationships
    const parentRelatedSchema = findSchemaByParentRelationship(fieldName, schema, form);
  ...
  if (formData) {
    const matchingSchema = findMatchingAnyOfSchema(schemas, formData as SchemaType, true);
    if (matchingSchema) {
      return matchingSchema;
    }
  }
