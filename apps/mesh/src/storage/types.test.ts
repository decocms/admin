...
describe('Database Types', () => {
  describe('Permission format', () => {
    it('should allow valid Permission format', () => {
      ...
      expect(permission['conn_abc123']).toEqual(['SEND_MESSAGE', 'LIST_THREADS']);
  ...
  describe('MCPConnection scoping', () => {
    ...
    it('should support all connection types', () => {
      const httpConn: Pick<MCPConnection, 'connectionType'> = { connectionType: 'HTTP' };
      const sseConn: Pick<MCPConnection, 'connectionType'> = { connectionType: 'SSE' };
      const wsConn: Pick<MCPConnection, 'connectionType'> = { connectionType: 'Websocket' };
