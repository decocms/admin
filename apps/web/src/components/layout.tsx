...
import { SidebarInset, SidebarProvider, useSidebar } from '@deco/ui/components/sidebar.tsx';
...
import { DecopilotChat, DecopilotTabs, toggleDecopilotTab } from './decopilot/index.tsx';
...
export function BaseRouteLayout({ children }: { children: ReactNode }) {
  ...
  const rootContext: Workspace = teamSlug ? `shared/${teamSlug}` : `users/${user?.id}`;
