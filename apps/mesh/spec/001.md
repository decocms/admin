...
8. **Hierarchical URL Structure** (inspired by Kubernetes resource scoping):
   - `/mcp` - Organization management (cluster-scoped operations)
   - `/mcp/:connection` - MCP proxy to organization-scoped services (available to all projects)
...
export class AccessControl {
   ...
   * 
   * @param resources - Optional connection IDs or tool names to check (OR logic)
...
#### 5. API Key Authentication via Better Auth

The Mesh uses [Better Auth's API Key plugin](https://www.better-auth.com/docs/plugins/api-key) for authentication, providing built-in permission management, rate limiting, and expiration:
...
const result = await auth.api.createApiKey({
  body: {
    ...
    permissions: {
      // Global MCP permissions
      'mcp:tools': ['read', 'execute'],
      'mcp:connections': ['read', 'write'],
      'mcp:projects': ['read'],
...
```typescript
// ✅ CORRECT: Using actual connection UUIDs
{
  // Global MCP permissions (applies to all connections)
  "mcp:tools": ["read", "execute"],
  "mcp:connections": ["read", "write"],
  "mcp:projects": ["read"],
  ...
  "project:proj_abc123": ["read", "write"],
}

// ❌ INCORRECT: Don't use friendly names
{
...
#### 7. MCP OAuth 2.1 Authorization Layer

The Mesh implements the [MCP Authorization Specification](https://modelcontextprotocol.io/specification/draft/basic/authorization) to act as an OAuth 2.1 authorization server for downstream MCP connections, providing a secure proxy layer regardless of whether the downstream MCP has OAuth support.

**Architecture:**

```typescript
// The Mesh acts as THREE OAuth roles simultaneously:
...
// 2. RESOURCE SERVER (for its own management API)
//    - Validates tokens using Better Auth JWT plugin
//    - Protects MCP tools with OAuth scopes
...
The Mesh exposes OAuth metadata to help MCP clients discover authorization endpoints:

```typescript
// auth/mcp-oauth-metadata.ts
...
app.get('/.well-known/oauth-protected-resource', async (c) => {
  ...
  return c.json({
    ...
    authorization_servers: [ctx.baseUrl], // We are our own authorization server
    scopes_supported: [
      'mcp:tools',           // Execute MCP tools
      'mcp:connections:read',  // Read connections
      'mcp:connections:write', // Create/update connections
      'mcp:projects:read',    // Read projects
      'mcp:projects:write',   // Create/update projects
    ],
    bearer_methods_supported: ['header'], // Authorization: Bearer <token>
    resource_documentation: `${ctx.baseUrl}/docs`,
  });
});
...
app.get('/.well-known/oauth-authorization-server', async (c) => {
  ...
  return c.json({
    ...
    introspection_endpoint: `${ctx.baseUrl}/oauth/introspect`,
    registration_endpoint: `${ctx.baseUrl}/oauth/register`, // Dynamic Client Registration
    scopes_supported: [
      'mcp:tools',
      'mcp:connections:read',
...
export { app as oauthMetadataRouter };
```

**Dynamic Client Registration (RFC7591):**

The Mesh supports automatic registration of MCP clients without manual configuration:

```typescript
// auth/oauth-register.ts
import { Hono } from 'hono';
import { z } from 'zod';
...
app.post('/oauth/register', async (c) => {
  ...
  const clientSecret = input.token_endpoint_auth_method === 'none' 
    ? undefined // Public client (PKCE only)
    : crypto.randomUUID(); // Confidential client
  
  // Store client in database (Kysely)
  const ctx = c.get('meshContext') as MeshContext;
  await ctx.db
    ...
    .values({
      id: crypto.randomUUID(),
      clientId,
      clientSecret: clientSecret ? await hashSecret(clientSecret) : null,
  ...
  // Return client credentials (RFC7591)
  return c.json({
    client_id: clientId,
...
export { app as oauthRegisterRouter };
```

**Token Introspection (for downstream MCPs):**

When proxying to downstream MCPs, the Mesh validates tokens using introspection:

```typescript
// auth/token-introspection.ts
interface TokenVerifier {
  verifyAccessToken: (token: string, requiredAudience: URL) => Promise<{
    token: string;
    clientId: string;
    scopes: string[];
    expiresAt: number;
...
export function createTokenVerifier(
  ...
  return {
    verifyAccessToken: async (token: string, requiredAudience: URL) => {
      ...
      const allowed = audiences.some(a => 
        checkResourceAllowed({ 
          requestedResource: a, 
          configuredResource: requiredAudience 
        })
      );
      
      if (!allowed) {
        throw new Error(
          `Token audience mismatch. Expected ${requiredAudience}, got: ${audiences.join(', ')}`
        );
      }
...
// Helper to check audience matches
function checkResourceAllowed(opts: { 
  requestedResource: string; 
  configuredResource: URL 
}): boolean {
  try {
    const requested = new URL(opts.requestedResource);
    return requested.origin === opts.configuredResource.origin;
  } catch {
    return false;
  }
}
...
When MCP clients make unauthorized requests, the Mesh returns proper OAuth challenges:
...
app.post('/:project/mcp/:connection', requireMcpAuth(['mcp:tools']), async (c) => {
  const { connection: connectionId } = c.req.param();
  const ctx = c.get('meshContext');
...
const MESH_URL = 'https://mesh.example.com/my-project/mcp';
...
async function connectToMesh() {
  ...
  authUrl.searchParams.set('scope', 'mcp:tools mcp:connections:read');
...
export function createMeshContextFactory(
  ...
  return async (c: Context): Promise<MeshContext> => {
    ...
    if (projectSlug) {
      ...
      // Verify API key has project-specific permissions
      if (auth.apiKey) {
        const hasProjectAccess = 
          auth.apiKey.permissions[`mcp:projects`]?.includes('read') ||
...
6. **Security**:
   ...
   - Better Auth's API Key plugin with built-in permissions, rate limiting, and secure hashing
   - UUID-based connection permissions prevent name-collision attacks
   - Resource-based permission model aligns with OAuth/OIDC standards
...
- **`ctx.access.check("TOOL_NAME")`** - Check if user has access to specific tool, grants if allowed
- **`ctx.access.check("TOOL1", "TOOL2")`** - Check if user has access to ANY of the tools (OR logic), grants if allowed
...
- `.check()` performs permission validation against Better Auth permissions and calls `.grant()` internally if allowed
...
// Resources can be:
//   - Global: "mcp:tools", "mcp:connections", "mcp:projects"
//   - Connection-specific: "conn_<UUID>" (actual connection ID)
// Example: {
//   "mcp:tools": ["read", "execute"],
