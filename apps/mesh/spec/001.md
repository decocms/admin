...
interface MCPConnection {
  ...
  // Allows MCPs to declare configuration needs and reference other connections
  configurationState?: Record<string, unknown>; // Encrypted in database, decrypted at runtime
  configurationScopes?: string[]; // Array of scope strings in format "KEY::SCOPE"
...
POST /mcp/tools/CONNECTION_CONFIGURE
Authorization: Bearer <api-key-or-oauth-token>
{
  "connectionId": "conn_abc123",
  "state": {
    "GMAIL": { "value": "conn_xyz789", "type": "@deco/gmail" }
  },
  "scopes": ["GMAIL::GetCurrentUser", "GMAIL::SendEmail"]
}
// Returns: { 
//   success: true, 
//   connectionId: "conn_abc123", 
//   configuredScopes: ["GMAIL::GetCurrentUser", "GMAIL::SendEmail"]
// }
```

The `CONNECTION_CONFIGURE` tool allows MCPs to declare configuration needs and reference other connections. When a connection has configuration:

1. The state is encrypted and stored in the database
2. The proxy automatically issues a short-lived JWT (5 minutes) with permissions derived from scopes
3. The JWT is sent to the downstream MCP via the `x-mesh-token` header
4. The JWT includes the full state in its metadata field

**Scope Format**: `"KEY::SCOPE"` where:
- `KEY` is a key in the state object
- `state[KEY].value` contains a connection ID (`conn_...`)
- The proxy issues a JWT with permission `{ "conn_xyz789": ["GetCurrentUser", "SendEmail"] }`

This enables MCPs to:
- Depend on other MCPs without requiring separate authentication
- Access cross-MCP functionality through a unified token
- Maintain security through validated connection references
