...
export type BinderImplementation<TBinder extends Binder<any>> = TBinder extends
  Binder<infer TDefinition> ? {
...
export const bindingClient = <TDefinition extends readonly ToolBinder[]>(
  ...
  return {
    implements: async (
      ...
      const listedTools = Array.isArray(connectionOrTools) ? connectionOrTools : await client
      ...
      return binder.every((tool) => (listedTools ?? []).some((t) => t.name === tool.name));
    },
    forConnection: (
...
export type MCPBindingClient<T extends ReturnType<typeof bindingClient>> = ReturnType<
  T['forConnection']
>;
