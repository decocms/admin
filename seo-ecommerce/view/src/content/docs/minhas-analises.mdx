...
<script>
(function(){
  const root = document.getElementById('saved-analises-root');
  let page=1, limit=10, total=0, q='';
  async function load(){
    // Supabase token
    const supaAuth = localStorage.getItem('la-supa-auth');
    let token=null, email=null;
    if(supaAuth){ try { const parsed = JSON.parse(supaAuth); token=parsed.access_token; email = parsed.user?.email || parsed.user?.user_metadata?.email; } catch {} }
    if(!token){ root.innerHTML = `<p>Você não está logado. Faça login na Home.</p>`; return; }
    const params = new URLSearchParams({ userHash: 'me', page:String(page), limit:String(limit) });
    if(q) params.set('q', q);
    const res = await fetch('/api/analises?'+params.toString(), { headers: token? { Authorization: 'Bearer '+token } : {} });
    if(!res.ok){ root.innerHTML='<p>Erro ao carregar análises.</p>'; return; }
    const data = await res.json();
    total = data.total;
    render(email || 'Usuário', data.items, token);
  }
  function render(user, items, token){
    ...
    root.querySelectorAll('.ma-del').forEach(btn=>btn.addEventListener('click', async e=>{
      const id = e.currentTarget.getAttribute('data-id');
      await fetch('/api/analises?userHash=me&id='+id, { method:'DELETE', headers: token? { Authorization: 'Bearer '+token } : {} });
      load();
    }));
