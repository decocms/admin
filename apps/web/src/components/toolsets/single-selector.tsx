...
import { Dialog, DialogContent, DialogHeader } from '@deco/ui/components/dialog.tsx';
...
function IntegrationList({
  ...
  if (isLoading) {
    return <div className='p-4 text-muted-foreground'>Loading integrations...</div>;
  }
  if (filtered.length === 0) {
    return <div className='p-4 text-muted-foreground'>No integrations found.</div>;
  }
  return (
    <>
...
function ToolList({ integration, value, search, onSelect }: ToolListProps) {
  ...
  const filtered = useMemo(
    () =>
      !toolsData ? [] : !search ? toolsData.tools : toolsData.tools.filter(
        (tool) =>
