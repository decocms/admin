...
interface ToolRequest { tool?: string; input?: any }
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		if (body.tool !== 'LINK_ANALYZER') return json({ error: 'tool desconhecida' }, 400);
		const url = body.input?.url?.toString() || '';
		if (!url || !/^https?:\/\//i.test(url)) return json({ error: 'URL inválida' }, 422);
		...
		try {
			...
			const proxyRes = await fetch(upstream, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url }) });
			if (proxyRes.ok) {
				const data = await proxyRes.json().catch(()=>null);
				if (data) return json({ tool: 'LINK_ANALYZER', result: data });
			}
			// If not ok, continue to stub
		} catch (_e) { /* ignore and stub */ }
...
function safeHost(u: string){
	try { return new URL(u).hostname; } catch { return 'host-desconhecido'; }
}
