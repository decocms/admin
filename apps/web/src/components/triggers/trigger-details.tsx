    ...
    <div className='flex flex-col gap-4 max-w-full py-8 h-full'>
      ...
      <ScrollArea className='min-h-0'>
        <div className='flex flex-col gap-4 px-4'>
          ...
          {trigger.data.type === 'webhook' && <WebhookDetails trigger={trigger.data} />}
          {trigger.data.type === 'cron' && <CronDetails trigger={trigger.data} />}
...
function TriggerIcon({ type }: { type: Trigger['type'] }) {
  return (
    <div className='flex items-center justify-center p-2 bg-primary/10 rounded-md'>
      {type === 'cron' && <Icon name='calendar_today' className='text-primary' />}
