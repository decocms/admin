...
var NEVER = INVALID;
// src/bindings/site.ts
var ContentSchema = exports_external.object({
  id: exports_external.string(),
  type: exports_external.string()
}).passthrough();
var ContentTypeSchema = exports_external.object({
  name: exports_external.string(),
  description: exports_external.string().optional(),
  schema: exports_external.any()
});
var BlockInputTypeSchema = exports_external.enum(["static", "dynamic"]);
var ContentBlockSchema = exports_external.object({
  id: exports_external.string(),
  contentType: exports_external.string(),
  viewId: exports_external.string().optional(),
  inputType: BlockInputTypeSchema,
  input: exports_external.record(exports_external.any()).optional()
});
var PageSchema = exports_external.object({
  id: exports_external.string(),
  path: exports_external.string(),
  title: exports_external.string().optional(),
  blocks: exports_external.array(ContentBlockSchema),
  metadata: exports_external.record(exports_external.any()).optional()
});
var ViewDescriptorSchema = exports_external.object({
  id: exports_external.string(),
  contentType: exports_external.string(),
  description: exports_external.string().optional()
});
var SITE_BINDING = {
  ...
  tools: {
    ...
    LIST_CONTENT_TYPES: {
      ...
      inputSchema: exports_external.object({}),
      outputSchema: exports_external.object({
        types: exports_external.array(ContentTypeSchema)
      }),
      cache: {
        public: true,
        ttlSeconds: 3600
    ...
    SEARCH_CONTENT: {
      ...
      inputSchema: exports_external.object({
        ...
        filters: exports_external.record(exports_external.any()).optional(),
        limit: exports_external.number().optional(),
        cursor: exports_external.string().optional()
      }),
      outputSchema: exports_external.object({
        items: exports_external.array(ContentSchema),
    ...
    LIST_VIEWS: {
      ...
      inputSchema: exports_external.object({}),
      outputSchema: exports_external.object({
        views: exports_external.array(ViewDescriptorSchema)
      }),
      cache: {
        public: true,
        ttlSeconds: 3600
...
// src/mcp/cache.ts
function generateCanonicalUrl(baseUrl, toolName, input, version) {
...
`).filter((line) => line.startsWith("data: "));
      ...
      if (method === "POST") {
        if (data2.error) {
          throw new Error(`MCP JSON-RPC Error: ${data2.error.message}`);
        }
        if (data2.result?.content?.[0]?.text) {
          console.log("Parsing MCP result content...");
          try {
            const parsed = JSON.parse(data2.result.content[0].text);
            ...
            return data2.result.content[0].text;
          }
        }
        if (data2.result) {
          console.log("Handling direct result");
          if (data2.result.structuredContent && typeof data2.result.isError === "boolean") {
            console.log("Unwrapping structuredContent from direct result");
            return data2.result.structuredContent;
          }
          return data2.result;
        }
...
function createSiteClient(config) {
  return new SimpleMcpClient(config);
}
// examples/client-only/app.ts
var MCP_URL = "http://localhost:3000/mcp";
var AUTH_TOKEN = "";
...
var client2 = createSiteClient({ baseUrl: MCP_URL, token: AUTH_TOKEN });
