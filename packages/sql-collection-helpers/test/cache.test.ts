...
describe("IntrospectionCache", () => {
  ...
  describe("basic operations", () => {
    ...
    it("should handle schema parameter in cache key", () => {
      ...
      const withSchema = cache.get(
        "postgres",
        "postgresql://localhost/test",
        "public",
      );
      const withoutSchema = cache.get(
        "postgres",
        "postgresql://localhost/test",
      );
    ...
    it("should clear all cache entries", () => {
      ...
      expect(
        cache.get("postgres", "postgresql://localhost/test1"),
      ).toBeUndefined();
  ...
  describe("staleness detection", () => {
    it("should detect when revalidation is needed for stale entries", async () => {
      ...
      // Initially not stale
      expect(
        cache.needsRevalidation("postgres", "postgresql://localhost/test"),
      ).toBe(false);
